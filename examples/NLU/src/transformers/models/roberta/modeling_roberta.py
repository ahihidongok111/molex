    ...
    def __init__(self, config):
        ...
        if self.config.use_gate and self.config.use_indv_gate:
            for layer_module in self.layer:
                ######## Expert choice is cosine gate #########
                if config.expert_choice:
                    print("COSINE GATE INIT")
                    layer_module.gate = CosineTopKGate(config.hidden_size, config.num_hidden_layers, proj_dim=config.proj_dim)
                ###############################################
                else:
                    layer_module.gate = nn.Linear(config.hidden_size, config.num_hidden_layers)
            ...
            if config.expert_choice:
                print("COSINE GATE INIT")
                self.gate = CosineTopKGate(config.hidden_size, config.num_hidden_layers, proj_dim=config.proj_dim)
            ###############################################
            else:
                self.gate = nn.Linear(config.hidden_size, config.num_hidden_layers)
        if self.config.use_learn_weight == 1 and not self.config.use_indv_gate:
        ...
        self.expert_choice = config.expert_choice
        ...
        for i, layer_module in enumerate(self.layer):
            if i <= self.layers_to_use:
                if self.use_indv_gate:
                    layer_out_all = layer_module.gate(hidden_states)
                else:
            ...
            layer_module_other =  self.layer[j]
            layer_head_mask_other = head_mask[j] if head_mask is not None else None
            past_key_value_other = past_key_values[j] if past_key_values is not None else None
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                if i <= self.layers_to_use:
                    ...
                    layer_outputs_other = layer_module_other(
                        hidden_states,
                        attention_mask,
                        layer_head_mask_other,
                        encoder_hidden_states,
                        encoder_attention_mask,
                        past_key_value_other,
                        output_attentions,
                    )
                    layer_outputs = tuple()
                    if self.use_indv_gate:
                        if isinstance(layer_module.weight_main, torch.Tensor):
                            exp_weight_main = torch.exp(layer_module.weight_main)
                            exp_weight_other = torch.exp(layer_module.weight_other)
                            denom = exp_weight_main + exp_weight_other
                            for ind, output in enumerate(layer_outputs_main):
                                layer_outputs += ((exp_weight_main/denom) * output + (exp_weight_other/denom) * layer_outputs_other[ind],)
                        else:
                            for ind, output in enumerate(layer_outputs_main):
                                layer_outputs += (layer_module.weight_main * output + layer_module.weight_other * layer_outputs_other[ind],)
                    elif self.use_gate and isinstance(self.weight_main, torch.Tensor):
                        ...
                        for ind, output in enumerate(layer_outputs_main):
                            layer_outputs += (self.weight_main * output + self.weight_other * layer_outputs_other[ind],)
                else:
