...
class ModelArguments:
    ...
    use_gate: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to apply gate for mixing or not."},
    )
    layers_to_use: Optional[int] = field(
        default=12,
        metadata={"help": "Number of layers for the model"},
    )
    use_learn_weight: Optional[int] = field(
        default=0,
        metadata={"help": "Whether to learn weights or not."},
    )
    expert_choice: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to use expert choice routing or not."},
    )
    use_load_balance: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to use load balancing or not."},
    )
    weight_main_init: Optional[float] = field(
        default=0.5,
        metadata={"help": "Initialise weight main"},
    )
    weight_other_init: Optional[float] = field(
        default=0.5,
        metadata={"help": "Initialise weight other"},
    )
    proj_dim: Optional[int] = field(
        default=256,
        metadata={"help": "Proj dim for cosine router"},
    )
    use_indv_gate: Optional[bool] = field(
        default=False,
        metadata={"help": "Whether to use individual routing or not."},
    )

@dataclass
class WandbArguments:
    """
    Arguments pertaining to wandb.
    """
    project_name: str = field(
        metadata={"help": "wandb project name"}
    )
    job_name: str = field(
        metadata={"help": "wandb job name"}
    )
    wandb_flag: bool = field(
        default=False, metadata={"help": "to use wandb or not"}
    )
...
def main():
    ...
    if not wandb_args.wandb_flag:
        print(wandb_args.wandb_flag)
        os.environ["WANDB_DISABLED"] = True
    wandb.init(project=wandb_args.project_name)
    wandb.run.name = wandb_args.job_name
    ...
    config = AutoConfig.from_pretrained(
        ...
        layers_to_use=model_args.layers_to_use,
        use_learn_weight=model_args.use_learn_weight,
        expert_choice=model_args.expert_choice,
        use_load_balance=model_args.use_load_balance,
        weight_main_init=model_args.weight_main_init,
        weight_other_init=model_args.weight_other_init,
        proj_dim=model_args.proj_dim,
        use_indv_gate=model_args.use_indv_gate,
    )
    ...
    trainable_params.append('gate')
    trainable_params.append('weight_main')
    trainable_params.append('weight_other')
