...
def add_optimizer_params(parser: argparse.ArgumentParser):
    ################################### MoLEx ####################################
    parser.add_argument('--gate_lr', default=0.01, type=float, help='The initial learning rate for AdamW for gate.')
    parser.add_argument('--gate_weight_decay', default=0.1, type=float, help='Weight decay for AdamW if we apply some for gate.')
    parser.add_argument('--g_balance', default=0.01, type=float, help='Weight for load balancing loss.')
    ##############################################################################
...
def create_grouped_parameters(model, no_decay_bias, gate_lr, gate_weight_decay):
    gate = ["gate", "weight_main", "weight_other"]
    if not no_decay_bias:
        optimizer_grouped_parameters = [
        {
            "params": [p for n, p in model.named_parameters() if not any(nd in n for nd in gate)], # if not any(nd in n for nd in no_decay)],
        },
        {
            "params": [p for n, p in model.named_parameters() if any(nd in n for nd in gate)],
        ...
        optimizer_grouped_parameters = [
        {
            "params": [p for n, p in model.named_parameters() if not any(nd in n for nd in total)],
        },
        {
...
def create_adam_optimizer_from_args(model, args, grouped_parameters=None):
    if grouped_parameters is None:
        grouped_parameters = create_grouped_parameters(model, args.no_decay_bias, args.gate_lr, args.gate_weight_decay)
